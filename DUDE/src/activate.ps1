...
# Helper function to block validation hosts
function Block-IDMValidation {
    ...
    try {
        if (Test-Path $hostsPath) {
            ...
            foreach ($entry in $entries) {
                ...
                $pattern = "(?m)^\s*(127\.0\.0\.1|0\.0\.0\.0)\s+" + [regex]::Escape($hostName)
                if ($content -notmatch $pattern) {
                    Add-Content -Path $hostsPath -Value "`r`n$entry" -Force
                }
            }
            Write-Host "Hosts file updated to block validation servers." -ForegroundColor Green
        }
    } catch {
        Write-Host "Warning: Failed to update hosts file ($($_.Exception.Message))" -ForegroundColor Yellow
    }
}
...
Set-ItemProperty -Path $regPath -Name "LstCheck" -Value "" -Force
...
# Block validation servers via hosts file
Block-IDMValidation

# Trigger IDM to initialize CLSID keys first
Trigger-IDMInitialization -Path $idmPath

# 3. Find and lock CLSID trial registry keys to freeze the activated state
Write-Host "Locking CLSID keys to prevent fake serial detection..." -ForegroundColor Cyan
